function       [a_post_snr,a_priori_snr   ]=estimate_snrs(noisy_dft_frame,fft_size, noise_psd ,SNR_LOW_LIM,  ALPHA  ,I,clean_est_dft_frame)  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%% 
 a_post_snr =  noisy_dft_frame(1:fft_size/2+1).*conj(noisy_dft_frame(1:fft_size/2+1))./(noise_psd(1:fft_size/2+1));%a posteriori SNR
if I==1,
   ...
   a_priori_snr=max(ALPHA*(abs(  clean_est_dft_frame(1:fft_size/2+1)).^2)./(   noise_psd(1:fft_size/2+1))+(1-ALPHA)*(a_post_snr-1),SNR_LOW_LIM);
end