...
def test_auditory_filter_bank_plot():
    ...
    auditory_filter_bank_freq_sum = sum(np.abs(auditory_filter_bank_freq)) 
    axs.semilogx(frequencies, mapping.signal_to_pressure(1 / auditory_filter_bank_freq_sum))
    axs.semilogx(frequencies, threshold_in_quiet_db(frequencies))

    axs.set_title("Frequency Domain Auditory Filters")
    axs.set_xlabel("frequency [Hz]")
    axs.set_ylabel("amplitude [dB]")
    axs.set_xlim(min(frequencies[1:]), max(frequencies)) 

    fig.tight_layout()
    fig.savefig("plots/test_auditory_filter_bank_plot_inverted.png")
